...
export async function up(knex: Knex): Promise<void> {
  // Check if account_code column exists, if not add it
  const hasAccountCode = await knex.schema.hasColumn('email_accounts', 'account_code');
...
export async function down(knex: Knex): Promise<void> {
  // Remove account_code field from email_accounts table if it exists
  const hasAccountCode = await knex.schema.hasColumn('email_accounts', 'account_code');
